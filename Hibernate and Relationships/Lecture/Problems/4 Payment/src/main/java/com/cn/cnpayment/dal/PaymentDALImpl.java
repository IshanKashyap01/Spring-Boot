...
public class PaymentDALImpl implements PaymentDAL{
	...
	{
		Session session = manager.unwrap(Session.class);
		return session.createQuery("select r from PaymentReview r where r.payment.id =:id", PaymentReview.class)
	...
	{
		Session session = manager.unwrap(Session.class);
		return session.createQuery("select distinct p from Payment p join p.paymentReviews r where lower(r.queryType) = :type", Payment.class)
		.setParameter("type", queryType.toLowerCase())
		.getResultList();
	}
